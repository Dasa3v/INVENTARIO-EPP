...
document.getElementById('recuperar-form').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (response.ok) {
            alertContainer.innerHTML = '<div class="alert alert-success">Contraseña actualizada correctamente. Redirigiendo al login...</div>';
            ...
            alertContainer.innerHTML = `<div class="alert alert-error">${data.detail || 'Error al actualizar contraseña'}</div>`;
        }
    } catch (error) {
